...
def get_variables_available_in_checkpoint(variables, checkpoint_path):
  ...
  # 如果未提供变量，则直接返回checkpoint中的变量
  elif variables == None:
    variable_names_map = {ckpt_var: ckpt_reader.get_tensor(ckpt_var) for ckpt_var in list(ckpt_vars)}
    
    return variable_names_map.values()
  else:
...
def get_init_fn(flags):
  ...
  """
  if flags.fine_tune_checkpoint is None:
      return None
  ...
  if tf.gfile.IsDirectory(flags.fine_tune_checkpoint):
      checkpoint_path = tf.train.latest_checkpoint(flags.fine_tune_checkpoint)
  else:
      checkpoint_path = flags.fine_tune_checkpoint
