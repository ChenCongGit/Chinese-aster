...
def random_pixel_value_scale(image, minval=0.9, maxval=1.1, seed=None):
  ...
  with tf.name_scope('RandomPixelValueScale', values=[image]):
    ...
    image = tf.clip_by_value(image, 0.0, 255.0) #1.0)
...
def random_adjust_brightness(image, max_delta=0.2):
  ...
  with tf.name_scope('RandomAdjustBrightness', values=[image]):
    ...
    image = tf.clip_by_value(image, clip_value_min=0.0, clip_value_max=255.0) # 1.0)
    return image
...
def image_to_float(image):
  """Used in FChinese_aster R-CNN. Casts image pixel values to float.
...
def random_rotate(image, max_angle=3):
  """
  随机旋转一定的角度
  Args:
    image: Rank 3 uint8 tensor containing 1 image -> [height, width, 3]
    max_angle: 最大旋转角度
  ...
  with tf.name_scope('RandomRotate', values=[image]):
    ...
    rotated_image = tf.py_func(random_rotate_func, [image], tf.uint8)
    rotated_image = tf.to_float(rotated_image)
...
def get_default_func_arg_map():
  prep_func_arg_map = {
      ...
      random_rotate: (fields.InputDataFields.image,),
