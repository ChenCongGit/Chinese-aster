...
from Chinese_aster.core import model
...
class MultiPredictorsRecognitionModel(model.Model):
  ...
  def predict(self, resized_images, scope=None):
    predictions_dict = {}
    #print('resized_images_size:', resized_images.shape)
    if self._spatial_transformer:
      ...
      #resized_images = tf.image.resize_images(resized_images, size=(32, 128))
  ...
  def postprocess(self, predictions_dict, scope=None):
    ...
    return recognition_text_list, recognition_scores_list  #aggregated_recognition_dict
  ...
  def provide_groundtruth(self, groundtruth_lists, scope=None):
    with tf.variable_scope(scope, 'ProvideGroundtruth', list(groundtruth_lists.values())):
      ...
      groundtruth_text_with_blank = tf.stack(
        groundtruth_lists[fields.InputDataFields.groundtruth_text_with_blank], axis=0)
      for name, predictor in self._predictors_dict.items():
        predictor.provide_groundtruth(
          groundtruth_text_with_blank,
