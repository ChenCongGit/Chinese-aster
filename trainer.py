...
from Chinese_aster.core import preprocessor
from Chinese_aster.core import batcher
from Chinese_aster.core import standard_fields as fields
from Chinese_aster.builders import preprocessor_builder
from Chinese_aster.builders import optimizer_builder
from Chinese_aster.utils import variables_helper
from Chinese_aster.utils import model_deploy
from Chinese_aster.utils import profile_session_run_hooks
...
def _get_inputs_multiqueues(input_queue_list):
  ...
  for input_queue in input_queue_list:
    ...
    transcript_with_blank_list.extend([
      read_data[fields.InputDataFields.groundtruth_text_with_blank] for read_data in read_data_list
    ])
  ...
  return images_list, {
    ...
    fields.InputDataFields.groundtruth_text_with_blank: transcript_with_blank_list,
    fields.InputDataFields.groundtruth_keypoints: keypoints_list
  }


def _create_losses(input_queue, create_model_fn, global_summaries):
  ...
  global_summaries.add(tf.summary.image('input_images', images))
...
def train(create_tensor_dict_fn_list, create_model_fn, train_config, master, task,
  ...
  with tf.Graph().as_default():
    ...
    # 定义了一个偏函数model_fn，函数_create_losses的create_model_fn参数固定
    model_fn = functools.partial(_create_losses, create_model_fn=create_model_fn, global_summaries=global_summaries)
    ...
    if train_config.fine_tune_checkpoint:
      init_fn = variables_helper.get_init_fn(train_config)
      ...
      # init_saver = tf.train.import_meta_graph(train_config.fine_tune_checkpoint + '.meta')
      # def initializer_fn(sess):
      #   init_saver.restore(sess, tf.train.latest_checkpoint('./Chinese_aster/experiments/demo/res_log_300k/'))
    ...
    #   save_summaries_secs=train_config.save_summaries_steps,
    #   saver=saver,
    #   save_interval_secs=keep_checkpoint_every_n_hours,
