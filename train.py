...
from Chinese_aster.protos import train_pb2
from Chinese_aster.protos import input_reader_pb2
from Chinese_aster.builders import model_builder
from Chinese_aster.builders import input_reader_builder
from Chinese_aster import trainer
...
fh = logging.FileHandler('./Chinese_aster/experiments/demo/log/train.log')
...
def get_configs_from_exp_dir():
  pipeline_config_path = os.path.join(FLAGS.exp_dir, 'config/ocr_train.prototxt')
...
def main(_):
  ...
  # 创建网络模型，从prototxt文件中获得模型的配置信息，使用model_builder.build创建模型
  ...
  create_input_dict_fn_list = [functools.partial(input_reader_builder.build, inpcfg) for inpcfg in input_config]
